...
export const getUfoById = async id => {
    return await execUsingMongoClient(async client => {
        const ufo = await client.collection('ufos').findOne({ _id: mongodb.ObjectId(id) });
        return ufo;
    });
};
...
export const updateUfo = async (ufo, id) => {
    return await execUsingMongoClient(async client => {
        console.log(await client.collection('ufos').updateOne({ _id: mongodb.ObjectId(id) }, {
            $set: {
                name: ufo.name,
...
export const deleteUfo = async id => {
    return await execUsingMongoClient(async client => {
        const result = await client.collection('ufos').deleteOne({ _id: mongodb.ObjectId(id) });
...
export const countSummary = async () => {
    return await execUsingMongoClient(async client => {
        return client.collection('ufos').aggregate([
            {
