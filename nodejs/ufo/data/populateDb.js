const ufoCollection = require('./collection');
...
(async () => {
    ...
    if (await ufoTypes.countDocuments() === 0) {
        const result = await ufoTypes.insertMany(ufoCollection.filter((value, index, self) => self.indexOf(self.find(s => s.type === value.type)) === index).map(u => ({ type: u.type })));
    ...
    if (await ufos.countDocuments() === 0) {
        ...
        const result = await ufos.insertMany(ufoCollection.map(u => {
            return {
                name: u.name,
